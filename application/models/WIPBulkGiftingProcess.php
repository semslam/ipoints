...
class WIPBulkGiftingProcess extends MY_Model {   
    ...
    protected function __map($value){
        ...
        $data->wallet_id = $value['wallet_id'];
    ...
    */
    ...
    public static function load($data, $verbose = FALSE){
        try
        {
            ...
            //$wallet_ = Wallet::walletById(data['wallet_id'][0]);
            // $loop = 1;
            // foreach($data as $dt){
            //     if(is_float($dt['qty']) || !$dt['qty'] > 0 || $wallet_->wallet_id != $dt['wallet_id']){
                    
            //         log_message('info',"The system can't accept Zero or Decimal number as qty value, OR One of the wallet contain deferent wallet id, At Line: ".$loop);
            ...
            try
            {
                ...
                foreach($chunks as $chunk){
                    $loop=1;
                    foreach($chunk as $chunk_dt){
                        if(is_float($chunk_dt['qty']) || !$chunk_dt['qty'] > 0){
    ...
    //private static function process($request, $verbose = FALSE){
        ...
        try {
            // $queueR =  new static();
            // $channel = $queueR->connection->channel(); 
            ...
            // $request = WIPBulkTransferRequest::findOne([
            ...
            //    ]);
                ...
                switch ($giftConfig->process_type) {
                    case 'default':
                        return SELF::defaultBulkGiftingProcess($request,$giftConfig,$batch,$msg);
                        break;
                    case 'espi':
                        return SELF::espiBulkGiftingProcess($request,$giftConfig,$batch, $msg);
                        break;
                    default:
    ...
    public static function defaultBulkGiftingProcess($request,$giftConfig,$batch,$msg){
                            ...
                            try{
                                ...
                                if($user['process']){
                                    ...
                                    if(!empty($userAnnualCharges) && $userAnnualCharges->status){
                                        ...
                                        $annualResult = UserSubscription::anuualsub($user['id'],$_reference,$batch[$x]->qty,$user['status'],$userAnnualCharges->value);
                                        ...
                                        $batch[$x]->qty = $annualResult['userbalance'];
                                    ...
                                    if(!empty($wallet_charge) && $userChargesByName->status){
                                        ...
                                        $isavings = Transaction::charges_proccess($batch[$x]->qty,$userChargesByName->value);
                                        ...
                                        $batch[$x]->qty = $userBalance;
                                        echo 'Wallet commission taking place============'.$userBalance.PHP_EOL;
                                    }
                                    echo 'Amount credited========================'.$batch[$x]->qty.PHP_EOL;
                                    $process = Transaction::credit(
                                        $user['id'],
                                        $batch[$x]->qty,
                                        $batch[$x]->wallet_id,
                                    ...
                                    (empty($template))?false:($giftConfig->send_message)?SELF::messageProcess($giftConfig,$user['id'],$process,$batch[$x]->id,$user['status'],$batch[$x]->recipient_phone,$batch[$x]->qty,$amountpaid, $commission,$template,$defaultPass):false;
    ...
    public static function espiBulkGiftingProcess($request,$giftConfig,$batch,$msg){
                        ...
                        try{
                            ...
                            if($user['process']){
                                ...
                                $espi_bulk_batches[] = array(
                                    ...
                                    'amount'=>round($batch[$x]->qty, 2, PHP_ROUND_HALF_DOWN),
                                    'ref'=>$_reference
                                );
                                $batch_wip[] = array(
                                    'id'=>$batch[$x]->id,
    ...
    public static function depositTransaction($walletId,$senderId,$payment_ref,$amount,$commission,$recipient_count){
           ...
           return EspiTransaction::depositOnBulkTransferProcess($senderId,'',$payment_ref,$amount,$commission,$recipient_count,'deposit',$description,FALSE);
    }
